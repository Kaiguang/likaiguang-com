...
function NavBarLink({ path, children }: { path: string; children: ReactNode }) {
  return (
    <GatsbyLink
      to={path}
      className='flex items-end text-lg font-light h-10 ml-1 sm:ml-2 px-2 pb-[2px] border-0 rounded-md border-transparent hover:bg-kaibluelight hover:text-gray-100 hover:fill-gray-100'
      activeClassName='bg-gradient-to-b from-kaiblue to-kaibluelight text-gray-100 fill-gray-100'
    >
      {children}
    </GatsbyLink>
  );
